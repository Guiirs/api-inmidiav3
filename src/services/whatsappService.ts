    ...
    async initialize() {
        try {
            ...
            this.client.on('qr', (qr) => {
                logger.info('[WhatsApp] 📱 QR Code gerado. Escaneie com seu WhatsApp:');
                console.log('\n========================================');
...
💡 O relatório diário é enviado automaticamente todos os dias às ${process.env.WHATSAPP_REPORT_HOUR || '09:00'}h
    ...
    async gerarRelatorio(empresaId = null) {
        try {
            ...
            if (alugueisAtivos.length > 0) {
                logger.debug(`[WhatsApp] Aluguéis ativos (primeiros 3):`, 
                    alugueisAtivos.slice(0, 3).map(a => ({
                        placa: a.placa?.numero_placa || a.placa,
            ...
            logger.error(`[WhatsApp] Erro ao gerar relatório: ${error.message}`);
            console.error(error);
            throw error;
        }
    }
    ...
    async enviarRelatorioDisponibilidade(chatId = null) {
        try {
            ...
            logger.error(`[WhatsApp] Erro ao enviar relatório: ${error.message}`);
            return false;
        }
    }
     ...
     */
    async notificarNovoAluguel(aluguel, placa, cliente) {
        try {
            ...
            logger.error(`[WhatsApp] Erro ao enviar notificação de aluguel: ${error.message}`);
            return false;
        }
    }
